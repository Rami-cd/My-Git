...
def main(argv=sys.argv[1:]):
    ...
    match args.command:     
        ...
        case "ping"         : cmd_ping(args)
        case _              : print("invalid command.")


argsp = argsubparsers.add_parser("ping",
                                help = "Ping")
...
def repo_path(repo, *path):
    """Compute path under repo's gtidir"""
...
def repo_file(repo, *path, mkdir=False):
    ...
    # this iterate over the dir name but the last name
...
def repo_default_config():
    ...
    # file mode permission traking
    ret.set("core", "filemode", "false")
    # working directory or not (here not bare)
...
def cmd_init(args):
    # this will take the command line argument aka the path and create the .git repository
...
def object_write(obj, repo=None):
    ...
    data = obj.serialize()
    # Add header
    result = obj.fmt + b' ' + strlen(len(data)).encode() + b'\x00' + data
    # Compute hash
...
def cat_file(repo, obj, fmt=None):
    obj = object_read(repo, args.object, fmt=fmt)
    # send binary to the standard output
...
# more simple than git hash-object command
...
def log_graphviz(repo, sha, seen):
    ...
    # Ensure special character preservasion
...
# We have to sort the tree objects since no 2 similar objects can have the same hash
# by git identity rule
...
argsp = argsubparsers.add_parser("ls-tree", help="Pretty-print a tree point.")
...
def ls_tree(repo, ref, recursive=None, prefix=""):
    ...
    for item in obj.items:
        ...
        match type: # Determine the type.abs
            case b'04': type = "tree"
        ...
        else: # This is a branch, recursive
...
# We search the subdirectories of refs for all the SHA1 identifiers
...
# these are tag objects unlike lightweight tags, tag objects have author, date, optional
# PGP signature and optional annotation. same formate as commite object.
# so we can just use subclass of commit object with change to the fmt
...
argsp.add_argument("--gclone-type",
                   metavar="type",
                   dest="type",
                   choices=["blob", "commit", "tag", "tree"],
                   default=None,
...
def index_read(repo):
    ...
    assert version == 2, "gclone only supports index file version 2"
